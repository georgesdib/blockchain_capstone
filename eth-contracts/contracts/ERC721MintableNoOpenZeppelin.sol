...
abstract contract Ownable {
    ...
    event OwnershipTransferred(address indexed oldOwner, address indexed newOnwer);
...
abstract contract Pausable is Ownable {
    bool private _paused; // false by default
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != owner, "Should not be the owner of the token");

        require(msg.sender == getOwner() || isApprovedForAll(owner, to),
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        emit Transfer(address(0), to, tokenId); // Emit from address 0 when minting
    }

    // @dev Internal function to transfer ownership of a given token ID to another address.
    // TIP: remember the functions to use for Counters. you can refresh yourself with the link above
    function _transferFrom(address from, address to, uint256 tokenId) internal virtual {
        require(ownerOf(tokenId) == from, "from has to be the owner of the token");
...
contract CapstoneERC721TokenCustom is ERC721Metadata {
    constructor()
        ERC721Metadata("Capstone", "CAP", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {}
